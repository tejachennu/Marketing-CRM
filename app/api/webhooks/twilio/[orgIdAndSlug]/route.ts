  ...
  try {
    ...
    if (
      ...
      try {
        ...
        console.log(`[Webhook Chatbot] Synonym expansion - Vector query: "${vectorQuery}", Keyword query: "${keywordQuery}"`)
          ...
          if (embedding) {
            ...
            try {
              ...
              try {
                ...
                console.error('[Webhook Chatbot] Vector RPC error:', vecErr)
              }
              ...
              try {
                ...
                console.error('[Webhook Chatbot] Keyword fallback search error:', kwErr)
              }
              ...
              console.log(`[Webhook Chatbot] Hybrid search merged ${faqs.length} FAQs (vector: ${vectorFaqs.length}, keyword: ${keywordFaqs.length})`)
            } catch (err) {
              console.error('[Webhook Chatbot] Search failed, falling back to simple keyword search:', err)
              try {
                faqs = await fallbackKeywordSearch(supabase, keywordQuery, orgId)
              } catch (kwFallbackErr) {
                console.error('[Webhook Chatbot] Ultimate keyword fallback failed:', kwFallbackErr)
              }
            }
                  ...
                  try {
                    ...
                    if (!existingTicket) {
                      ...
                      if (newTicket) {
                        ...
                        }).catch(err => console.error('[Webhook Notification] Failed to trigger ticket notification:', err))
                      }
                    }
                  } catch (ticketErr: any) {
